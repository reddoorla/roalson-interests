...
| [`PropertyMap.svelte`](../src/lib/components/PropertyMap.svelte) | `points`, `label`, `engine`, `tone`, `active`, `onselect` | 31 | The comp's per-section property map (#13), with its ground tones EXPORTED from this module script as `MAP_TONES` |
...
81 modules, 706 tests behind them.
