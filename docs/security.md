...
Since 2026-09-23 the map's **first state is a committed raster of that same style** — `static/map-home-*.webp`, generated by `pnpm map:home` (#122) and guarded by `scripts/map-home.test.ts`, which fails when `static/map-style.json` changes without a regenerate. It costs **no CSP change at all**: the files are same-origin and `img-src 'self'` already covers them, so the map still reaches exactly one external host. It is worth saying why that is a security note and not only a performance one — the placeholder is a picture of **our** style at **our** camera, so nothing a third party deploys can change what a visitor sees before the tiles arrive.
